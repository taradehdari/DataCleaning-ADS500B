...
A tetrachoric correlation is performed on deposit and age group, job, marital, and education. A tetrachoric correlation takes the correlation of binary and categorical variables. Through the correlations we can see that there are no high correltions between the categorical variables, and earlier we noticed how there were no high correlations between the numerical values as well. 
...
#print frequency tables in table format
...
#30-49 age is highest which is why there are more deposits in that age group to fix 
#this we are going to normalize the counts by the total number of clients in each age group,this way the resulting count will take into account that there are different number of clients in each age group.
...
## Exploring Logistic Regression to get a final visualization and model
```{r warning=FALSE}
...
do a logistic regression with poutcome, jobs and deposit as dependent variable (y)
then say final model but not best model cuz nothing works 
